...
--
-- This migration does not fetch remote sources from PostgreSQL.
-- A trusted server worker fetches official sources and calls the server-only RPC.
-- Browser roles cannot mutate normative baselines or approval-enabling receipts.

create table if not exists public.civic_education_normative_source_baselines (
  id uuid primary key default gen_random_uuid(),
  source_key text not null,
  authority text not null
    check (authority in ('MIM','NORMATTIVA','GAZZETTA_UFFICIALE')),
  title text not null,
  ...
  check (
    (authority = 'MIM'
      and lower(source_url) ~ '^https://([a-z0-9-]+\.)*mim\.gov\.it([/:?#]|$)')
    or
    (authority = 'NORMATTIVA'
      and lower(source_url) ~ '^https://([a-z0-9-]+\.)*normattiva\.it([/:?#]|$)')
    or
    (authority = 'GAZZETTA_UFFICIALE'
      and lower(source_url) ~ '^https://([a-z0-9-]+\.)*gazzettaufficiale\.it([/:?#]|$)')
  ),
...
alter table public.civic_education_normative_check_receipts
  ...
  add column if not exists baseline_set_fingerprint text
    check (
      baseline_set_fingerprint is null
      or baseline_set_fingerprint ~ '^[a-f0-9]{64}$'
    );
...
revoke all on public.civic_education_normative_source_baselines
  from public, anon, authenticated;
revoke all on public.civic_education_normative_source_heads
  from public, anon, authenticated;

grant select, insert on public.civic_education_normative_source_baselines
  to service_role;
grant select, insert, update on public.civic_education_normative_source_heads
  to service_role;
...
for each row
execute function public.reject_civic_education_normative_baseline_mutation_v1();
...
begin
  ...
  -- Stable retry identity: concurrent retries serialize before the lookup.
  ...
  if p_verification_snapshot->'automaticCheck' is distinct from 'true'::jsonb
     ...
     or p_verification_snapshot->>'verifiedFrameworkVersion'
          is distinct from p_framework_version_label
  ...
  begin
    ...
    v_human_confirmed_at :=
      (p_verification_snapshot->>'humanConfirmedAt')::timestamptz;
  ...
  -- Stabilize the active normative head set while the receipt is recorded.
  ...
  join public.civic_education_normative_source_baselines baseline
  ...
  for v_observation in
    select value from jsonb_array_elements(p_source_observations)
  loop
    if jsonb_typeof(v_observation) <> 'object'
       ...
       or coalesce(v_observation->>'normalizationVersion', '')
            <> 'RAW_RESPONSE_BYTES_V1'
    ...
    v_observed_keys :=
      array_append(v_observed_keys, v_observation->>'sourceKey');
    ...
    if v_baseline.expected_sha256 <> v_observation->>'sha256'
       or v_baseline.normalization_version
            <> v_observation->>'normalizationVersion'
    ...
    if v_baseline.authority = 'MIM' then
      v_has_mim := true;
    end if;
    if v_baseline.authority in ('NORMATTIVA','GAZZETTA_UFFICIALE') then
      v_has_legal := true;
    end if;
    ...
    if v_source is null
       ...
       or (v_source->>'checkedAt')::timestamptz
            is distinct from v_checked_at then
...
begin
  ...
  into v_active_count, v_current_baseline_set_fingerprint
  from public.civic_education_normative_source_heads head
...
for each row
execute function public.enforce_current_civic_normative_receipt_v1();
