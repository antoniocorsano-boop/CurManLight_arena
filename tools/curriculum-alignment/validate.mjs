...
assert(registry.version === '1.11.0', 'versione registro inattesa');
...
assert(sourceRepertory?.status === 'CURRENT_ALIGNED_TO_MASTER_1_1', 'repertorio fonti non allineato al master 1.1');
...
assert(currentSourceDomain.includes("alignmentState: 'ALIGNED_TO_MASTER_1_1'"), 'master non collegato al repertorio fonti');
...
assert(curriculumProcess?.source_traceability === 'ALIGNED_TO_MASTER_1_1', 'tracciabilità fonti non allineata');
...
assert(sourceLogic?.source_traceability_status === 'ALIGNED_TO_MASTER_1_1', 'runtime fonti non allineato');
...
for (const token of ['Versione:** 1.11', master.title, master.file_id, 'versione `1.1`', normativeMatrix.title, normativeMatrix.file_id, sourceRepertory.title, sourceRepertory.file_id, 'ALL-CURR-A@1.1', 'MATERIALIZZAZIONE COMPLETA', 'fonte primaria di provenienza', 'non genera un nuovo curricolo parallelo']) {
