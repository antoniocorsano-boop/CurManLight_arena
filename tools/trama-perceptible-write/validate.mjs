...
if (!base || !head || /^0+$/.test(base)) { console.log('TRAMA-PW-01 PASS: manifest validated; diff unavailable'); process.exit(0) }
...
const mutationPattern = /(onSubmit\s*=|<form|\.insert\s*\(|\.update\s*\(|\.delete\s*\(|\.upsert\s*\(|localStorage\.setItem|indexedDB|setItem\s*\(|\b(save|publish|approve|confirm|remove|delete|accept|dismiss|withdraw)[A-Z_a-z0-9]*\s*\()/i
const mutationFiles = source.filter((f) => mutationPattern.test(f.text))
if (!mutationFiles.length) { console.log('TRAMA-PW-01 PASS: no changed user-facing mutation candidate'); process.exit(0) }

const feedbackPattern = /(aria-live|role=["']status["']|role=["']alert["']|Toast|toast\s*\(|feedback|notice|pending|success|error|riuscit|fallit|salvat|aggiunt|rimoss|confermat|pubblicat|ritirat)/i
const feedbackFiles = source.filter((f) => feedbackPattern.test(f.text))
...
if (!tests.some((f) => feedbackPattern.test(f.text))) fail('mutation changed without automated test evidence for perceived feedback')

console.log(JSON.stringify({contract:'TRAMA-PW-01',result:'PASS',mutationFiles:mutationFiles.map(f=>f.path),feedbackFiles:feedbackFiles.map(f=>f.path),feedbackTests:tests.filter(f=>feedbackPattern.test(f.text)).map(f=>f.path)},null,2))
