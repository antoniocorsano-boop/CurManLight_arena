...
(async () => {
  ...
  try {
    ...
    const sourceCard = page.getByRole('heading', { name: 'kx3-worker-fixture', exact: false }).locator('xpath=ancestor::article[1]');
    ...
    await openKnowledgeSources(page);
    const persistedCard = page.getByRole('heading', { name: 'kx3-worker-fixture', exact: false }).locator('xpath=ancestor::article[1]');
