...
export const OPERATIONAL_GROUPS: readonly OperationalGroupDefinition[] = [
  {
    code: 'P-G01',
    order: 'primaria',
    label: 'Area linguistico-storico-geografica',
    disciplines: ['italiano', 'storia', 'geografia', 'religione'],
  },
  {
    code: 'P-G02',
    order: 'primaria',
    label: 'Area matematico-scientifico-tecnologica',
    disciplines: ['matematica', 'scienze', 'tecnologia'],
  },
  {
    code: 'P-G03',
    order: 'primaria',
    label: 'Area delle lingue straniere',
    disciplines: ['inglese'],
  },
  {
    code: 'P-G04',
    order: 'primaria',
    label: 'Area artistico-espressiva e motoria',
    disciplines: ['musica', 'arteImmagine', 'educazioneFisica'],
  },
  {
    code: 'S-G01',
    order: 'secondaria',
    label: 'Area linguistico-storico-geografica',
    disciplines: ['italiano', 'storia', 'geografia', 'latino', 'religione'],
  },
  {
    code: 'S-G02',
    order: 'secondaria',
  ...
  },
  {
    ...
    order: 'secondaria',
    label: 'Area delle lingue straniere',
    disciplines: ['inglese', 'secondaLingua'],
  },
  {
    code: 'S-G04',
    order: 'secondaria',
...
export const getOperationalGroupByCode = (
  code: string | null | undefined,
): OperationalGroupDefinition | null =>
  OPERATIONAL_GROUPS.find((group) => group.code === code) ?? null;

export const getOperationalGroupForDiscipline = (
  order: SchoolOrder,
  discipline: string,
): OperationalGroupDefinition | null => {
  if (discipline === 'educazioneCivica') return null;
  return getOperationalGroupsForOrder(order)
    .find((group) => group.disciplines.includes(discipline)) ?? null;
};

export const getOperationalGroupsForDisciplines = (
  order: SchoolOrder,
  disciplines: readonly string[],
): OperationalGroupDefinition[] => {
  const selected = new Set(disciplines);
  return getOperationalGroupsForOrder(order)
    .filter((group) => group.disciplines.some((discipline) => selected.has(discipline)));
};

export const getOperationalDisciplinesForOrder = (order: SchoolOrder): string[] =>
  Array.from(new Set(
    getOperationalGroupsForOrder(order).flatMap((group) => [...group.disciplines]),
  ));
...
/**
 * Educazione civica e AI Literacy non sono gruppi autonomi. Il routing viene
 * definito dalla singola formulazione/nucleo e resta esplicito e verificabile.
 */
