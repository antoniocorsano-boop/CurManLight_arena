    ...
    <div role="dialog" aria-modal="true" className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[160] flex items-center justify-center p-4">
     <div className="bg-white border max-w-lg w-full rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] md:max-h-[85vh] h-auto fade-in font-medium">
      <div className="bg-gradient-to-r from-primary-600 to-indigo-700 text-white px-6 py-4 flex justify-between items-center shrink-0">
        <span className="flex items-center space-x-2 font-black uppercase tracking-wider text-xs"><UserCog className="w-5 h-5" /> <span> Profilo personale locale</span></span>
      ...
      <div className="p-4 sm:p-5 overflow-y-auto flex-1 space-y-3 sm:space-y-4 text-xs text-slate-700 text-left">
       <div className="text-center space-y-1 pb-1">
        <h4 className="text-sm font-extrabold text-slate-800">
          {onboardingStep === 1 && "Passo 1: Ruolo dichiarato personalmente"}
          {onboardingStep === 2 && "Passo 2: Ordine per la consultazione"}
        ...
        <p className="text-[10px] text-slate-500 font-medium">
          {onboardingStep === 1 && "La scelta è locale, non autenticata e non assegna autorità istituzionale."}
          {onboardingStep === 2 && "Selezione personale per consultare il curricolo; non configura l'offerta dell'istituto."}
       ...
       {onboardingStep === 1 && (
        <div className="space-y-1.5 border border-slate-200 p-3 bg-slate-50 rounded-xl fade-in">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">Ruolo dichiarato localmente</label>
         <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 max-h-[140px] overflow-y-auto p-1 bg-white rounded border border-slate-200">
           {(['non-dichiarato', 'insegnante', 'dipartimento', 'referente', 'dirigente', 'collegio', 'amministratore'] as UserRole[]).map(r => (
         ...
         {onboardingRole === 'insegnante' && (
          <div className="pt-2 border-t mt-2 space-y-1.5 text-xs font-bold text-slate-700">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">Tipologia di attività dichiarata</label>
       ...
       {onboardingStep === 2 && (
        <div className="space-y-2 border border-slate-200 p-4 bg-slate-50 rounded-2xl font-bold fade-in">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">Ordine selezionato per consultazione personale</label>
       ...
       {onboardingStep === 4 && (
        <div className="fade-in space-y-3">
         ...
         <div className="p-3 bg-indigo-50/50 border border-indigo-150 rounded-2xl space-y-2 text-xs">
           ...
           <p className="text-[9px] text-slate-500 font-normal leading-normal">Inserisci solo le sezioni utili alla sessione personale. Questa scelta non configura o rappresenta l'istituto.</p>
          ...
          <div className="space-y-1.5 pt-1 text-left">
           <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block">Sezioni dichiarate localmente:</span>
             ...
             <div key={idx} className="flex items-center space-x-1 bg-slate-50 hover:bg-white border border-slate-200 px-2 py-1 rounded-lg text-[10px] font-bold shadow-sm transition">
              ...
              {availableSections.length > 1 && (
               <button 
                ...
                title="Elimina questa sezione locale"
               >
          ...
          <div className="flex justify-end pt-1">
           <button 
            onClick={() => {
             ...
             setOnboardingCombinations(filteredCombos);
             showToast("Esempi di sezione ripristinati localmente.");
            }} 
           ...
           >
            <span>Ripristina esempi locali</span>
           </button>
          </div>
         </div>
         ...
         {onboardingOrd === 'infanzia' ? (
          <div className="space-y-3">
           <div className="bg-indigo-50 border border-indigo-150 p-4 rounded-2xl text-[10px] leading-relaxed font-bold text-indigo-950">
            <span>Contesto personale trasversale:</span>
            <p className="font-normal mt-1 leading-normal text-slate-700">Hai dichiarato un'attività trasversale. Questa selezione locale non assegna una cattedra o una disponibilità dell'istituto.</p>
           </div>
...
export function SaveSettingsModal({
  ...
  const handleDialogKeyDown = (event: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    const focusable = Array.from(dialogRef.current?.querySelectorAll<HTMLElement>('button, input, select, textarea, [tabindex]:not([tabindex="-1"])') ?? [])
      .filter(element => !element.hasAttribute('disabled') && element.getAttribute('type') !== 'hidden' && !element.closest('dialog:not([open])'));
  ...
  return (
    <>
    <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="save-settings-title" onKeyDown={handleDialogKeyDown} className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[160] flex items-center justify-center p-4">
     <div className="bg-white border max-w-lg w-full rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] fade-in text-slate-700 text-left">
      <div className="bg-slate-900 text-white px-5 py-3 flex justify-between items-center shrink-0 border-b border-slate-800">
        <span id="save-settings-title" className="font-black uppercase tracking-wider text-[11px] flex items-center space-x-1.5">
      ...
      <div className="flex border-b bg-slate-50 shrink-0 overflow-x-auto text-[10px] font-black uppercase text-slate-500">
       <button onClick={() => { setShowOnboardingModal(true); setShowCloudAccountModal(false); setShowAgentSetupModal(false); setShowSaveModal(false); }} className="flex-1 py-3 px-2 border-b-2 text-center transition min-w-[80px] border-transparent hover:bg-slate-100 hover:text-slate-700">Profilo personale</button>
       ...
       <button onClick={() => { setShowOnboardingModal(false); setShowCloudAccountModal(false); setShowAgentSetupModal(false); setShowSaveModal(true); }} className="flex-1 py-3 px-2 border-b-2 text-center transition min-w-[80px] border-indigo-600 text-indigo-700 bg-white font-extrabold">Memoria locale</button>
      </div>
      ...
      <div className="p-5 space-y-4 text-xs leading-relaxed text-slate-700 text-left overflow-y-auto flex-1">
         ...
         <div className="space-y-2">
          ...
          <button onClick={() => { handleDownloadBackup(); setShowSaveModal(false); }} className="w-full p-2 bg-white hover:bg-slate-50 border border-slate-200 rounded-lg flex items-center space-x-2.5 text-left font-bold shadow-sm transition">
           ...
           <div className="space-y-0.5">
            ...
            <div className="text-[9px] text-slate-400 font-medium">Scarica i dati locali in formato JSON</div>
           </div>
          </button>
         </div>
        </div>
         ...
         <div className="space-y-2">
          <label className="w-full p-2 bg-white hover:bg-slate-50 border border-slate-200 rounded-lg flex items-center space-x-2.5 text-left font-bold shadow-sm transition cursor-pointer">
           ...
           <div className="space-y-0.5">
            ...
            <div className="text-[9px] text-slate-400 font-medium">Carica un file JSON locale validato</div>
           </div>
          ...
          <button onClick={() => { setShowResetConfirm(true); setShowSaveModal(false); }} className="w-full p-2 bg-white hover:bg-red-50 border border-slate-200 hover:border-red-200 rounded-lg flex items-center space-x-2.5 text-left font-bold shadow-sm transition group">
           ...
           <div className="space-y-0.5">
            <div className="text-[11px] text-slate-800 group-hover:text-red-700">Azzera dati locali</div>
            <div className="text-[9px] text-slate-400 font-medium">Svuota i dati memorizzati nel browser</div>
           </div>
          </button>
         </div>
        </div>
       ...
       <div className="p-4 border-2 border-indigo-100 bg-indigo-50/10 rounded-2xl space-y-3">
        <div className="flex items-center justify-between">
         <span className="text-[9px] font-black text-indigo-600 uppercase tracking-wider block">Copia JSON su Google Drive</span>
         {isWorkspaceLoggedIn && (
        ...
        <p className="text-[10px] text-slate-500 font-semibold leading-relaxed">
          Copia automatica delle programmazioni e delle UDA come file JSON nel tuo spazio <strong>Google Drive / OneDrive</strong>. L'applicazione non cifra il file caricato.
        </p>

        <div className="bg-white border rounded-xl p-3 space-y-1.5 text-left">
         <label className="text-[8px] font-black text-slate-400 uppercase tracking-wider block">ID client Google OAuth:</label>
         <div className="flex gap-2 items-center">
          <input 
           ...
           className="border border-slate-200 rounded px-2.5 py-1.5 font-mono text-[9px] flex-1 outline-none bg-slate-50 focus:bg-white text-slate-700 focus:ring-1 focus:ring-indigo-500" 
            placeholder="Inserisci il Client ID Google OAuth..."
          />
          <button 
           onClick={() => {
             setWorkspaceClientId('');
             safeLocalStorageSetItem('curman_workspaceClientId', '');
             showToast("Client ID rimosso.");
           }} 
        ...
        {isWorkspaceLoggedIn ? (
         ...
         >
          <span>Accedi a una sessione Google Drive</span>
         </button>
        )}
       </div>

       <div className="p-4 border-2 border-rose-100 bg-rose-50/10 rounded-2xl space-y-2.5 text-left">
        <span className="text-[9px] font-black text-rose-700 uppercase tracking-wider block">Recupero dei dati locali</span>
        <p className="text-[10px] text-slate-500 leading-relaxed">
         Se i dati locali risultano vuoti dopo pulizia della cache, navigazione privata o errori del browser, puoi tentare il ripristino da una copia locale:
        </p>
...
export function DocumentViewModal({
  ...
  const institutionalHeaderLines = [
    institutionalProjection.primaryHeading,
    institutionalProjection.displayName,
    ...institutionalProjection.secondaryLines,
  ].filter((line): line is string => Boolean(line));
  const clipboardText = [
    ...institutionalHeaderLines,
    '',
    generatedDocText,
    ...(institutionalProjection.footer ? ['', institutionalProjection.footer] : []),
  ].join('\n');
  return (
     ...
     <div className="bg-white border max-w-2xl w-full rounded-2xl shadow-2xl overflow-hidden flex flex-col h-[80vh] fade-in text-left font-medium">
      ...
      <div className="p-6 overflow-y-auto flex-1 bg-slate-100 flex justify-center shadow-inner">
       <div className="bg-white border w-full max-w-2xl p-8 sm:p-12 rounded-xl shadow-lg text-slate-800 text-xs leading-relaxed text-left font-sans space-y-6 select-text border-slate-200">
         {institutionalProfile.warning && <div role="status" className="border border-amber-300 bg-amber-50 text-amber-900 rounded-lg px-3 py-2 font-semibold">{institutionalProfile.warning} Puoi comunque esportare in modalità personale o dimostrativa.</div>}
          <div className="border-b-2 border-indigo-600 pb-3 text-[9px] text-slate-500 space-y-1">
           <div className="space-y-0.5">
            {institutionalProjection.primaryHeading && <p className="font-semibold">{institutionalProjection.primaryHeading}</p>}
            <strong className="block text-slate-800 text-[10px]">{institutionalProjection.displayName}</strong>
            {institutionalProjection.secondaryLines.map(line => <p key={line}>{line}</p>)}
           </div>
         </div>

        <pre className="whitespace-pre-wrap font-sans text-slate-700 leading-relaxed text-xs">{generatedDocText}</pre>

         {institutionalProjection.footer && <div className="border-t border-slate-200 pt-4 text-center text-[10px] text-slate-500">{institutionalProjection.footer}</div>}
       </div>
      </div>
       ...
       >
        ...
        <button onClick={() => {
         copyText(clipboardText);
          showToast("Documento locale copiato negli appunti con successo!", true);
