...
import { AlertTriangle, CheckCircle2, CircleDotDashed, RefreshCw, Route } from 'lucide-react';
...
export function InstituteSourceChangeTracePanel() {
  ...
  const conflicts = trace.filter((entry) => entry.status === 'CONFLICT').length;
    ...
    <section className="rounded-2xl border border-indigo-200 bg-indigo-50/50 p-4 sm:p-5" data-source-change-trace>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="max-w-3xl">
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-xs font-bold">
            ...
            {conflicts > 0 && <span className="rounded-full bg-rose-100 px-2.5 py-1 text-rose-800">Conflitti {conflicts}</span>}
      ...
      <details className="mt-4 rounded-xl border border-indigo-100 bg-white">
        ...
        <div className="border-t border-indigo-100 p-4 space-y-3">
          {trace.map((entry) => {
            const statusClass = entry.status === 'RESOLVED'
            ...
            const statusLabel = entry.status === 'RESOLVED'
              ? 'Risolto'
              : entry.status === 'CONFLICT'
                ? 'Conflitto'
                : entry.status === 'ACKNOWLEDGED'
            ...
            return (
              <article key={entry.taskId} className="rounded-xl border border-slate-200 bg-slate-50 p-4" data-source-change-trace-entry={entry.taskId}>
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-black ${statusClass}`}>
