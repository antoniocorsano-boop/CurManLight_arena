...
export function AppSidebar(props: AppSidebarProps) {
  ...
  const closeMobileNavigation = () => {
    if (window.innerWidth < 768) {
      window.dispatchEvent(new CustomEvent('arena:mobile-navigation-closed'));
    }
  };

  const switchTab = (tab: string) => {
    props.handleTabSwitch(tab);
      ...
      <nav className="space-y-5 text-left" aria-label="Navigazione principale">
        ...
        <section aria-labelledby="nav-curricolo" className="border-t border-slate-100 pt-4">
          ...
          <div className="space-y-1">
            <button
              type="button"
              onClick={() => { props.setActiveCurricoloView('home'); switchTab('curricolo'); }}
            ...
            <button type="button" onClick={() => switchTab('revisione')} className={itemClass(props.activeTab === 'revisione')}>
          ...
          {props.activeTab === 'curricolo' && (
            <details className="mt-2 rounded-xl border border-slate-100 bg-slate-50" data-beta-secondary-navigation>
              ...
              <div className="space-y-1 border-t border-slate-100 p-2">
                <button type="button" onClick={() => { props.setActiveCurricoloView('albero'); closeMobileNavigation(); }} className="w-full rounded-lg px-3 py-2 text-left text-xs font-semibold text-slate-600 hover:bg-white">Struttura</button>
                <button type="button" onClick={() => { props.setActiveCurricoloView('mappa'); closeMobileNavigation(); }} className="w-full rounded-lg px-3 py-2 text-left text-xs font-semibold text-slate-600 hover:bg-white">Confronto nel tempo</button>
                <button type="button" onClick={() => { props.setActiveCurricoloView('popolamento'); closeMobileNavigation(); }} className="w-full rounded-lg px-3 py-2 text-left text-xs font-semibold text-slate-600 hover:bg-white">Aggiornamento</button>
              </div>
            </details>
          )}
        </section>
