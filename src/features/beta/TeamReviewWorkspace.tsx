...
export function TeamReviewWorkspace({ proposals, decisions, customTexts }: TeamReviewWorkspaceProps) {
  ...
  useEffect(() => {
    ...
    ]).then(([nextContributions, nextOutcomes]) => {
      if (!active) return;
  ...
  const summary = useMemo(() => deriveTeamReviewSummary(descriptors, contributions), [descriptors, contributions]);
  ...
  const confirmSharedItems = async () => {
    ...
    const pendingShared = sharedItems.filter((item) => !latestOutcomes[item.proposalRef]);
    if (pendingShared.length === 0) {
      setMessage('Tutti i punti già condivisi hanno già un esito del team registrato per la versione corrente.');
      return;
    }
    const context: WorkspaceActorContext = { membership: selectedMembership, assurance: 'authenticated-workspace' };
    setBusy(true);
    ...
    try {
      for (const item of pendingShared) {
        ...
        await repository.recordTeamOutcome(context, {
          ...
          outcome,
          sharedText: null,
          rationale: 'Punto confermato dal team come già condiviso durante la riunione.',
          clientRequestId: createRequestId(),
        });
      }
      setRefreshVersion((value) => value + 1);
      setMessage(`${pendingShared.length} punti già condivisi registrati come esito del team senza riaprire la lettura delle schede.`);
    } catch (error) {
      ...
      {!session ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-2 lg:grid-cols-4" aria-label="Sintesi del lavoro del team">
            <div className="rounded-xl bg-white p-3"><strong className="block text-xl text-slate-900">{summary.shared}</strong><span className="text-xs text-slate-600">punti già condivisi</span><span className="mt-1 block text-[10px] text-slate-400">non serve rileggerli</span></div>
            ...
            <div className="rounded-xl bg-white p-3"><strong className="block text-xl text-slate-900">{summary.needsClarification}</strong><span className="text-xs text-slate-600">punti da chiarire</span><span className="mt-1 block text-[10px] text-slate-400">manca un contributo corrente</span></div>
          </div>
              ...
              <div className="space-y-2 border-t border-emerald-100 p-3">
                {sharedItems.map((item) => (
                  <div key={item.proposalRef} className="flex items-center justify-between gap-3 rounded-lg bg-white p-2 text-xs">
                    ...
                    <span className="shrink-0 text-[10px] font-semibold text-emerald-700">{item.contributionCount} contributi concordi</span>
                  </div>
                ))}
            ...
            {openDiscussionItems.length === 0 ? (
              ...
              <article key={item.proposalRef} className="rounded-xl border border-slate-200 bg-white p-3">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  ...
                  <span className="text-[10px] text-slate-500">{item.contributionCount} contributi correnti</span>
                </div>
