...
export function TeamReviewWorkspace({ proposals, decisions, customTexts }: TeamReviewWorkspaceProps) {
  ...
  const publishPreparation = async () => {
    ...
    try {
      ...
      setMessage('Non riesco a condividere il tuo lavoro con il team. Riprova.');
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <section aria-label="Lavoro del team" className="space-y-4 rounded-2xl border border-indigo-200 bg-indigo-50/30 p-4" data-team-meeting-workspace>
      ...
      {!session ? (
        ...
        <div role="status" className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">Il tuo accesso è attivo, ma non risulti ancora associato a un team. Chiedi al referente di verificare l’assegnazione.</div>
      ) : (
        <>
