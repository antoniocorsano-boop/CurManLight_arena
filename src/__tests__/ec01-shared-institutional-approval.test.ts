...
function expectSqlLexicallyBalanced(sql: string): void {
  let parens = 0;
  ...
  for (let i = 0; i < sql.length; i += 1) {
    ...
    if (char === '
  it('is lexically balanced and defines each EC authority block exactly once', () => {
    expectSqlLexicallyBalanced(migration);
    ...
    expect(migration).toContain("normative_fingerprint text not null check (normative_fingerprint ~ '^[a-f0-9]{64}
